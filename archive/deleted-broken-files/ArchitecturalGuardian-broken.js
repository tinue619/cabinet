...
export class ArchitecturalGuardian {
     ...
     * @private
     */
    _validateSectionSize(entity) {
        if (entity && entity.type === 'Section') {
            const minSize = CABINET_DNA.CONSTANTS.MIN_SECTION_SIZE;
            if (entity.width < minSize || entity.height < minSize) {
                return {
                    law: 'MINIMUM_SECTION_SIZE',
                    message: `Section ${entity.id} is smaller than minimum ${minSize}mm`,
                    severity: 'ERROR'
                };
            }
        }
     ...
     * @private
     */
    _validateMaterialConsistency(entity) {
        if (entity && entity.type === 'Panel') {
            if (!entity.material || !CABINET_DNA.MATERIALS[entity.material]) {
                return {
                    law: 'MATERIAL_CONSISTENCY',
                    message: `Panel has invalid material: ${entity.material}`,
                    severity: 'ERROR'
                };
            }
        }
     ...
     * @private
     */
    _validateSection2D(entity) {
        if (entity && entity.type === 'Section' && entity.hasProperty && entity.hasProperty('z')) {
            return {
     ...
     * @private
     */
    _validateEditingMode(entity, operation) {
