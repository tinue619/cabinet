...
class Renderer3D {
  ...
  setupControls() {
    ...
    if (!isMobile) {
      // Привязываем события мыши для десктопа
      ...
      // Для мобильных устройств touch-события обрабатываются в TouchHandler
      // Здесь добавляем только базовую поддержку, если TouchHandler не загружен
  ...
  setupBasicTouchControls() {
    let touchStartX = 0;
    let touchStartY = 0;
    let startRotation = { x: this.rotation.x, y: this.rotation.y };
    let startDistance = 0;
    let startZoom = this.distance;
    ...
    this.canvas.addEventListener('touchstart', (e) => {
      ...
      if (e.touches.length === 1) {
        ...
        startRotation = { x: this.rotation.x, y: this.rotation.y };
      } else if (e.touches.length === 2) {
        const dx = e.touches[0].clientX - e.touches[1].clientX;
        const dy = e.touches[0].clientY - e.touches[1].clientY;
        startDistance = Math.sqrt(dx * dx + dy * dy);
        startZoom = this.distance;
      }
    }, { passive: false });
    ...
    this.canvas.addEventListener('touchmove', (e) => {
      ...
      if (e.touches.length === 1) {
        // Одним пальцем - вращение
        const deltaX = e.touches[0].clientX - touchStartX;
        const deltaY = e.touches[0].clientY - touchStartY;
        
        this.rotation.y = startRotation.y - deltaX * 0.01;
        this.rotation.x = startRotation.x + deltaY * 0.01;
