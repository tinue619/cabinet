...
class Cabinet {
  ...
  getAllParts() {
    const parts = [
      { name: 'Боковина левая', w: CONFIG.PANEL_THICKNESS, h: this.height, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Боковина правая', w: CONFIG.PANEL_THICKNESS, h: this.height, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Дно', w: this.interiorWidth, h: CONFIG.PANEL_THICKNESS, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Крыша', w: this.interiorWidth, h: CONFIG.PANEL_THICKNESS, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Цоколь передний', w: this.interiorWidth, h: CONFIG.PANEL_THICKNESS, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Цоколь задний', w: this.interiorWidth, h: CONFIG.PANEL_THICKNESS, d: this.depth, material: 'ЛДСП 16мм' },
      { name: 'Задняя стенка', w: this.interiorWidth, h: this.height - this.base - CONFIG.PANEL_THICKNESS, d: 3, material: 'ХДФ 3мм' },
      { name: 'Фасад', w: this.width, h: this.height - this.base, d: CONFIG.PANEL_THICKNESS, material: 'МДФ 16мм' }
    ];
  ...
  // ===========================
  // DOMAIN LAYER: Добавление разделителей
  // ===========================
  addDivider(type, position, start, end) {
    // Валидация входных параметров
    if (type !== 'h' && type !== 'v') {
      console.error('Неверный тип разделителя:', type);
      return false;
    }
    ...
    const divider = {
      ...
      type: type, // 'h' = горизонтальный, 'v' = вертикальный
      pos: position, // позиция разделителя
      start: start, // начальная координата
      end: end // конечная координата
    };
    
    // Проверяем границы
    if (type === 'h') {
      // Горизонтальный разделитель (полка)
      if (position < CONFIG.MIN_SECTION || 
          position > this.interiorHeight - CONFIG.MIN_SECTION) {
        console.error('Полка выходит за допустимые границы');
        return false;
      }
    } else {
      // Вертикальный разделитель (стойка)
      if (position < CONFIG.MIN_SECTION || 
          position > this.interiorWidth - CONFIG.MIN_SECTION) {
        console.error('Стойка выходит за допустимые границы');
        return false;
      }
    }
    ...
    console.log('Разделитель добавлен:', divider);
    return true;
  }

  // ===========================
  // DOMAIN LAYER: Получение списка всех панелей
  // ===========================
  getAllParts() {
    const parts = [];
    
    // Обязательные панели согласно онтологии
    const baseParts = [
      {
        name: 'Боковина левая',
        w: CONFIG.PANEL_THICKNESS,
      ...
      {
        name: 'Дно',
        w: this.width - 2 * CONFIG.PANEL_THICKNESS,
        h: CONFIG.PANEL_THICKNESS,
        d: this.depth,
        material: 'ЛДСП 16мм',
        type: 'горизонтальная'
      ...
      },
      {
        name: 'Задняя стенка',
        w: this.width - 2 * CONFIG.PANEL_THICKNESS,
        h: this.height - this.base - CONFIG.PANEL_THICKNESS,
        d: 3,
        material: 'ХДФ 3мм',
        type: 'фронтальная'
      },
      {
        name: 'Фасад',
        w: this.width,
        h: this.height - this.base,
        d: CONFIG.PANEL_THICKNESS,
        material: 'МДФ 16мм',
        type: 'фронтальная'
      }
    ];
    ...
    // Добавляем дополнительные панели (разделители)
  ...
  // ===========================
  // APPLICATION LAYER: Подсчет общего объема материала
  // ===========================
  getTotalVolume() {
    ...
    parts.forEach(part => {
      ...
      const volumeMm3 = part.w * part.h * part.d;
      // Переводим в кубические метры
      totalVolume += volumeMm3 / (1000 * 1000 * 1000);
    });
  ...
  // ===========================
  // APPLICATION LAYER: Подсчет стоимости (базовая логика)
  // ===========================
  getTotalCost() {
    // Базовые цены за кубический метр (примерные)
    const materialPrices = {
      'ЛДСП 16мм': 25000, // руб/м³
      'ХДФ 3мм': 15000,   // руб/м³
      'МДФ 16мм': 35000   // руб/м³
    };
    ...
    parts.forEach(part => {
      const volumeM3 = (part.w * part.h * part.d) / (1000 * 1000 * 1000);
      const price = materialPrices[part.material] || 25000;
  ...
  // Получить ограничения для перемещения полки
  getShelfLimits(shelf) {
    if (!shelf || shelf.type !== 'h') return { min: 0, max: this.interiorHeight };
    
    let min = 0;
    let max = this.interiorHeight;
    
    // Ищем соседние полки сверху и снизу
    this.dividers.forEach(d => {
      if (d.type === 'h' && d.id !== shelf.id) {
        // Проверяем пересечение по горизонтали
        if (shelf.start < d.end && shelf.end > d.start) {
          if (d.pos < shelf.pos && d.pos > min) {
            min = d.pos + CONFIG.PANEL_THICKNESS;
          } else if (d.pos > shelf.pos && d.pos < max) {
            max = d.pos - CONFIG.PANEL_THICKNESS;
          }
        }
      }
    });
    
    // Учитываем минимальный размер секций
    min += CONFIG.MIN_SECTION;
    max -= CONFIG.MIN_SECTION;
    
    return { min, max };
  }

  // Получить ограничения для перемещения штанги
  getRodLimits(rod) {
    if (!rod) return { min: 0, max: this.interiorWidth };
    
    const section = this.sections[rod.sectionId];
    if (!section) return { min: 0, max: this.interiorWidth };
    
    return {
      min: section.x + 20,
      max: section.x + section.w - rod.length - 20
    };
  }
  ...
  moveDivider(dividerId, newPos) {
    ...
    if (divider.type === 'v') {
      const maxPos = this.interiorWidth - CONFIG.PANEL_THICKNESS;
      if (newPos < CONFIG.MIN_SECTION || newPos > maxPos - CONFIG.MIN_SECTION) {
        return false;
      }
    } else {
      const maxPos = this.interiorHeight - CONFIG.PANEL_THICKNESS;
